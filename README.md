...
<p align="center">
  <img src="https://img.shields.io/badge/versão-1.2.0-blue?style=for-the-badge&logo=python&logoColor=white" alt="versão" />
...
A biblioteca se conecta a um backend seguro que já possui uma conta Google pré-autorizada. Você fornece o link da pasta (com permissão de **editor**) e a biblioteca cuida de todo o resto: upload, download, exclusão, movimentação, renomeação e agora **criação de pastas e arquivos** dentro de pastas compartilhadas.
...
| **Listagem** | `drive.listar()` | Lista todos os arquivos e subpastas de uma pasta. Exige o link da pasta. |
| **Download** | `drive.download()` | Baixa arquivos individuais ou pastas inteiras (como `.zip`). |
...
| **Criar Pasta** | `drive.criar_pasta()` | Cria uma nova pasta **dentro de uma pasta compartilhada** (link obrigatório). |
| **Criar Arquivo** | `drive.criar_arquivo()` | Cria um arquivo (`.txt`, `.py`, `.java`, `.lua`, etc.) dentro de uma pasta compartilhada, com o conteúdo desejado. |

> **Importante:** Os métodos `upload()`, `listar()`, `criar_pasta()` e `criar_arquivo()` **exigem** o parâmetro `pasta_link` (ou `pasta_pai_link`). Sem ele, a biblioteca lançará um erro `ValueError`. Isso foi projetado intencionalmente para evitar que arquivos sejam acidentalmente enviados para a raiz da conta autorizada.
...
# Criar uma pasta dentro de uma pasta compartilhada
pasta = drive.criar_pasta("MinhaPasta", "https://drive.google.com/drive/folders/PASTA_PAI_ID")
print("Pasta criada:", pasta['folder_link'])

# Criar um arquivo Python dentro da nova pasta
arquivo = drive.criar_arquivo(
    "script.py", 
    "print('Olá, mundo!')", 
    pasta['folder_link']
)
print("Arquivo criado:", arquivo['file_link'])
...
### 2. Criar uma Pasta (dentro de uma pasta compartilhada)

```python
pasta = drive.criar_pasta(
    nome_pasta="Documentos",
    pasta_pai_link="https://drive.google.com/drive/folders/ID_DA_PASTA_PAI"
)
print(pasta['folder_link'])
```
...
drive.criar_arquivo(
    ...
    conteudo="Minhas anotações importantes",
    pasta_pai_link="https://drive.google.com/drive/folders/ID_DA_PASTA"
)
...
### 5. Listar Arquivos de uma Pasta
...
### 7. Excluir Arquivos ou Pastas

```python
drive.excluir("https://drive.google.com/file/d/ARQUIVO_ID/view")
```

### 8. Mover Arquivos ou Pastas
...
| Criar pastas e arquivos dentro de pastas compartilhadas | ✔️ |
...
**Causa:** Você tentou fazer upload, listar, criar pasta ou arquivo sem fornecer o link da pasta.  
**Solução:** Sempre inclua o parâmetro `pasta_link` ou `pasta_pai_link` com um link válido.
...
```bash
# Clone o repositório
git clone https://github.com/rex41788-arch/drive-editor-.git
cd drive-editor-
...
Se encontrar um bug ou tiver uma sugestão, [abra uma issue](https://github.com/rex41788-arch/drive-editor-/issues) com:
